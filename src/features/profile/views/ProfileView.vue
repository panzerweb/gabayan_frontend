...
const user = computed(() => {
  ...
  const fullName = data.name || (data.first_name ? `${data.first_name} ${data.last_name || ''}`.trim() : null) || 'User Name'
  ...
  return {
    ...
    avatar: data.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(fullName)}&background=0D8ABC&color=fff`,
    stats: [
...
<template>
  <div class="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <!-- Cover Image -->
      <div class="h-32 w-full bg-gradient-to-r from-green-500 to-emerald-600"></div>
      ...
      <div class="px-6 pb-6 relative">
        ...
        <div class="flex justify-between items-end -mt-12 mb-4">
          ...
          <button class="px-4 py-2 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 font-medium rounded-lg transition-colors text-sm border border-emerald-200">
            Edit Profile
          </button>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <div v-for="(stat, index) in user.stats" :key="index" class="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center">
        <p class="text-3xl font-bold text-gray-800">{{ stat.value }}</p>
        <p class="text-sm font-medium text-gray-500 mt-1 uppercase tracking-wider">{{ stat.label }}</p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      ...
      <div class="space-y-4">
        ...
        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors cursor-pointer">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="font-medium text-gray-800 text-red-600">Danger Zone</p>
      ...
      <!-- Logout Button -->
      <div class="mt-8">
        <button @click="handleLogout" class="w-full py-3 px-4 bg-red-50 text-red-600 font-medium rounded-xl border border-red-100 hover:bg-red-100 transition-colors flex items-center justify-center gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
